...
How to set up a TronGrid Lite build environment from scratch on Windows or Linux.
...
| Platform | Compiler | Minimum | Configure preset |
|----------|----------|---------|------------------|
| Windows | MSVC (Visual Studio 2022) | 19.30 | `windows-msvc` |
| Windows | Clang-CL (LLVM for Windows) | 15 | `windows-clang-cl` |
| Windows | MinGW-w64 GCC | 12 | `windows-mingw` |
| Linux | GCC | 12 | `linux-x11-gcc` |
| Linux | Clang | 15 | `linux-x11-clang` |
...
It should print something like `C:\VulkanSDK\1.3.296.0`.
...
Run `clang-format -i` on every changed `.cpp` and `.hpp` file. The repository's `.clang-format`
enforces Allman braces, 4-space indent and a 170-column limit.
...
Markdown files must pass markdownlint with the repository's `.markdownlint.json`.
