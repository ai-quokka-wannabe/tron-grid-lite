...
namespace
{
    ...
    //! Hash-based value noise. Deterministic, seeded, and free of any external dependency.
    [[nodiscard]] float valueNoise(float x, float z, uint32_t seed)
    {
        // Integer cell coordinates and the fractional position inside that cell.
        ...
        // Smoothstep the fractions so that the interpolation has a continuous first derivative
        // across cell boundaries; plain linear interpolation would leave visible creases.
        fx = (fx * fx) * (3.0f - (2.0f * fx));
        fz = (fz * fz) * (3.0f - (2.0f * fz));

        // Pseudo-random value in [0, 1] from a pair of integer coordinates. All arithmetic is
        // unsigned, because signed integer overflow would be undefined behaviour.
        const auto hash = [seed](int32_t px, int32_t pz) -> float {
        ...
        // Bilinear interpolation of the four cell corners.
    ...
    //! Sums several octaves of value noise, each at twice the frequency and half the amplitude of the last.
    ...
    */
    ...
    //! Builds the quantised heightmap sampled at every grid vertex, row-major in Z then X.
    [[nodiscard]] std::vector<float> buildHeightmap(const TerrainConfig& config)
    {
        const uint32_t verts_per_side{config.cells + 1u};
        const float half_size{(static_cast<float>(config.cells) * config.cell_size) * 0.5f};

        std::vector<float> heightmap(static_cast<std::size_t>(verts_per_side) * verts_per_side);

        for (uint32_t z{0u}; z < verts_per_side; ++z) {
            for (uint32_t x{0u}; x < verts_per_side; ++x) {
                const float world_x{(static_cast<float>(x) * config.cell_size) - half_size};
                const float world_z{(static_cast<float>(z) * config.cell_size) - half_size};
                heightmap[(static_cast<std::size_t>(z) * verts_per_side) + x] = terrainHeight(config, world_x, world_z);
            }
        }
...
float terrainHeight(const TerrainConfig& config, float world_x, float world_z)
{
    float height{layeredNoise(world_x, world_z, config.noise_octaves, config.noise_frequency, config.seed)};

    // Snap to discrete levels — this is what makes the terrain terrace instead of roll.
    if (config.quantise_levels > 0u) {
        const float levels{static_cast<float>(config.quantise_levels)};
        height = std::floor(height * levels) / levels;
    }

    return height * config.height_scale;
}

Mesh generateTerrain(const TerrainConfig& config)
{
    Mesh terrain{};

    const uint32_t verts_per_side{config.cells + 1u};
    const std::vector<float> heightmap{buildHeightmap(config)};

    const float half_size{(static_cast<float>(config.cells) * config.cell_size) * 0.5f};
    const float half_cells{static_cast<float>(config.cells) * 0.5f};

    const std::size_t triangle_count{static_cast<std::size_t>(config.cells) * config.cells * 2u};
    terrain.vertices.reserve(triangle_count * 3u);
    ...
    for (uint32_t z{0u}; z < config.cells; ++z) {
        for (uint32_t x{0u}; x < config.cells; ++x) {
            ...
            const float h00{heightmap[(static_cast<std::size_t>(z) * verts_per_side) + x]};
            const float h10{heightmap[(static_cast<std::size_t>(z) * verts_per_side) + x + 1u]};
            const float h01{heightmap[(static_cast<std::size_t>(z + 1u) * verts_per_side) + x]};
            const float h11{heightmap[(static_cast<std::size_t>(z + 1u) * verts_per_side) + x + 1u]};

            const MathLib::Vec3 p00{x0, h00, z0};
            const MathLib::Vec3 p10{x1, h10, z0};
            const MathLib::Vec3 p01{x0, h01, z1};
            const MathLib::Vec3 p11{x1, h11, z1};
            ...
            emitSurfaceQuad(terrain, p00, p10, p01, p11, u0, u1, v0, v1);
        }
    }
...
NeonGrid generateTerrainNeon(const TerrainConfig& terrain_config, const NeonTubeConfig& tube_config)
{
    NeonGrid neon{};

    const uint32_t verts_per_side{terrain_config.cells + 1u};
    const std::vector<float> heightmap{buildHeightmap(terrain_config)};

    const float half_size{(static_cast<float>(terrain_config.cells) * terrain_config.cell_size) * 0.5f};

    const auto gridVertex = [&](uint32_t gx, uint32_t gz) -> MathLib::Vec3 {
        const float world_x{(static_cast<float>(gx) * terrain_config.cell_size) - half_size};
        const float world_z{(static_cast<float>(gz) * terrain_config.cell_size) - half_size};
        return MathLib::Vec3{world_x, heightmap[(static_cast<std::size_t>(gz) * verts_per_side) + gx], world_z};
    };
    ...
    for (uint32_t z{0u}; z < verts_per_side; ++z) {
        ...
        for (uint32_t x{0u}; x < terrain_config.cells; ++x) {
            const bool major_edge{major_row || isMajorGridLine(x, tube_config.major_interval)};
            Mesh& target{major_edge ? neon.accent : neon.primary};
            emitEdgeQuad(target, gridVertex(x, z), gridVertex(x + 1u, z), tube_config);
        }
    ...
    for (uint32_t x{0u}; x < verts_per_side; ++x) {
        ...
        for (uint32_t z{0u}; z < terrain_config.cells; ++z) {
            const bool major_edge{major_column || isMajorGridLine(z, tube_config.major_interval)};
            Mesh& target{major_edge ? neon.accent : neon.primary};
            emitEdgeQuad(target, gridVertex(x, z), gridVertex(x, z + 1u), tube_config);
        }
