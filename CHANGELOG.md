...
- **Nothing runs without a reason.** The renderer no longer spins at the GPU's maximum rate redrawing
  ...
  12 s of wall clock against 7.56 s drawing continuously** — about eighty times less, with the GPU
  dropping off its clocks entirely. State comparison rather than dirty flags, deliberately: a flag is
  correct only if every writer remembers to raise it, and forgetting costs a window that has stopped
  updating. A swapchain rebuild explicitly forces the next frame, because its new images hold
  undefined contents and a resize that left the camera and size unchanged would otherwise look idle
  and leave garbage on screen. `--continuous` opts out, for profiling: the GPU profiler averages over
  frames and a still camera produces none.
